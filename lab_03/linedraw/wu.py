...
def wu(x_start, y_start, x_end, y_end, color):
    ...
    grad = dy/dx
    intery = y_start + rfpart(x_start) * grad
    ...
    def get_endpoint(x_s, y_s):
        x_e = round(x_s)
        y_e = y_s + grad * (x_e - x_s)
        x_gap = rfpart(x_s + 0.5)
        px, py = int(x_e), int(y_e)
        dens1 = rfpart(y_e) * x_gap
        dens2 = fpart(y_e) * x_gap
        dots_f.extend([[*p(px, py), color + (255 * dens2, 255 * dens2, 255)]])
        dots_s.extend([[*p(px, py), color + (255 * dens1, 255 * dens1, 255)]])
    ...
    for x in range(x_s, x_e):
        ...
        dens1 = rfpart(intery)
        dens2 = fpart(intery)
        dots_f.extend([[*p(x, y), color + (255 * dens2, 255 * dens2, 255)]])
        dots_s.extend([[*p(x, y+1), color + (255 * dens1, 255 * dens1, 255)]])
        intery += grad

    return dots_f, dots_s
